...
func (slog spanLogger) Info(msg string, fields ...zapcore.Field) {
	slog.logToSpan("info", msg, fields...)
	slog.logger.Info(msg, append(slog.spanFields, fields...)...)
}

func (slog spanLogger) Error(msg string, fields ...zapcore.Field) {
	slog.logToSpan("error", msg, fields...)
	ext.Error.Set(slog.span, true)
	slog.logger.Error(msg, append(slog.spanFields, fields...)...)
}

func (slog spanLogger) Fatal(msg string, fields ...zapcore.Field) {
	slog.logToSpan("fatal", msg, fields...)
	ext.Error.Set(slog.span, true)
	slog.logger.Fatal(msg, append(slog.spanFields, fields...)...)
}

func (slog spanLogger) With(fields ...zapcore.Field) Logger {
	return spanLogger{
		logger: slog.logger.With(fields...),
		span: slog.span,
		spanFields: slog.spanFields,
	}
}

func (slog spanLogger) logToSpan(level string, msg string, fields ...zapcore.Field) {
	fa := fieldAdapter(make([]log.Field, 0, len(fields) + 2))
	fa = append(fa, log.String("event", msg))
	fa = append(fa, log.String("level", level))
	for _, field := range fields {
		field.AddTo(&fa)
	}
	slog.span.LogFields(fa...)
}
